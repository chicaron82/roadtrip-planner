...
interface SmartTimelineProps {
  ...
  /** Unranked gas/hotel/restaurant/cafe corpus for Tier-2 hub discovery. */
...
export function SmartTimeline({ summary, settings, vehicle, poiSuggestions = [], poiInference }: SmartTimelineProps) {
  ...
  useEffect(() => {
    ...
    // Prefer the inference corpus (gas/hotel) for hub detection — it contains
    // the utility POIs that Tier-2 analyzeForHub needs regardless of whether
    // the user enabled those map categories. Fall back to curated suggestions.
    const hubPOIs = poiInference ?? poiSuggestions;

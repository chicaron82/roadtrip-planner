...
export function DiscoveryPanel({
  ...
  } = useDiscoveryController({ suggestions, onAdd, onAddMultiple });
    ...
    <div className={cn('rounded-xl border-2 border-amber-200 bg-gradient-to-br from-amber-50/80 to-orange-50/50', className)}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-amber-900">{title || 'Discover Cool Stops'}</h3>
          <span className="text-xs font-semibold text-amber-700 bg-amber-200 px-2 py-0.5 rounded-full">
            {visibleCount} found
          </span>
        </div>
        <div className="text-xs text-amber-600">{isExpanded ? '▲' : '▼'}</div>
      </button>
        ...
        <div className="px-4 pb-4 space-y-4">
          ...
          <div className="bg-white/70 rounded-lg p-3 border border-amber-200">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-semibold text-amber-900">🕐 Discovery Budget</label>
